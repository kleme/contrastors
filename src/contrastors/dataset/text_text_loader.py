...
def collate_local_ds(batch, tokenizer, add_prefix=False, query_only=None, path2prefix=None):
    ...
    #     try:
    #         compile(batch[i]['document'][0], '<string>', 'exec')
    #         pythons.append(i)
    #     except:
    #         continue
    ...
    tokenized_inputs = {}
    for col in keys:
        #collected = [sample[col] for i, sample in enumerate(batch) if i in pythons]
        collected = [sample[col] for sample in batch] #collected = [sample[col] for i, sample in enumerate(batch) if i in pythons]
        if isinstance(collected[0], list):
            collected = sum(collected, [])
    ...
    return {**tokenized_inputs, **{'dataset_name': ds_names[0]}}

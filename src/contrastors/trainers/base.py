...
from contrastors.dataset.text_code_loader import ProbabilisticDataLoader
...
class BaseTrainer(metaclass=ABCMeta):
    def __init__(self, config, dtype=torch.float32):
        ...
        if self.deepspeed:
            ...
            self.engine = model
            self.model = {**self.model, **{"model": model}}
    ...
    def get_tokenizer(self, config):
        ...
        if "codesage" in config.tokenizer_name:
            print("Adding EOS token into Tokenizer")
            tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_name, trust_remote_code=True, add_eos_token=True)
        else:
            tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_name, trust_remote_code=True)
        tokenizer.model_max_length = config.seq_len
        if config.skip_add_tok:
            return tokenizer
    ...
    def load_state(self, input_dir):
        ...
        try:
            ...
            print('Warning RNG states not loaded from checkpoint in multi-GPU DataParallel')    
        
        with open(f'{input_dir}/train_info.json', 'r') as file:
            train_info = json.load(file) 
        return train_info['epoch'], train_info['step']

    def save_state(self, output_dir, epoch, step, is_epoch = False):
        ...
        if isinstance(self.dataloaders['train'], ProbabilisticDataLoader):
            data_config = self.config.data_args
            ds_save_path = f'{output_dir}/saved_ds_state.json'
            if not is_epoch:
                self.dataloaders['train'].save_state(ds_save_path)
        ...
        train_info = dict(epoch = epoch + 1, step = 0)  if is_epoch else dict(epoch = epoch, step = step + 1) 
        with open(f'{output_dir}/train_info.json', 'w') as file:
    ...
    def train(self):
        ...
        if train_args.checkpoint:
            start_epoch, start_step = self.load_state(train_args.checkpoint)
            new_init_lr = self.optimizer.param_groups[0]['lr']
            for pi in range(len(self.optimizer.param_groups)):
                self.optimizer.param_groups[pi]['initial_lr'] = new_init_lr
                
            self.scheduler = get_scheduler(name=self.config.train_args.schedule_type, optimizer=optimizer, num_warmup_steps=0, num_training_steps=total_num_steps)
            scheduler = self.scheduler
            optimizer = self.optimizer
        ...
        for epoch in range(start_epoch, train_args.num_epochs):
            if epoch > 0 and getattr(data_config, "streaming", False) is False:
                ...
                elif epoch > 0 and data_config.text_code:
            ...
            progbar = tqdm(
                train_dataloader, desc=f"Epoch {epoch}", disable=not self.global_rank == 0, total=total_num_steps + start_step, 
                initial=start_step
            )
            ...
            with context() as p:
                for step, batch in enumerate(progbar):
                    if batch.get('skip_batch', False):
                        continue
                    # skip_batch = batch.get('skip_batch', False)
                    # if dist.is_initialized():
                    #     skip_flag_tensor = torch.tensor([int(skip_batch)], device=f"cuda:{torch.distributed.get_rank()}")
                    #     dist.all_reduce(skip_flag_tensor, op=dist.ReduceOp.SUM)
                    #     skip_batch = skip_flag_tensor.item() > 0  
                    # if skip_batch:
                    #     continue
                    ...
                    if step > 0 and train_args.save_every > 0 and step % train_args.save_every == 0:
                        self.save_state(f"{train_args.output_dir}/step_{curr_step}", epoch, step)
            ...
            if train_args.save_every > 0:
                ...
                if train_args.num_epochs > 1:
                    self.save_state(f"{train_args.output_dir}/epoch_{epoch}", epoch, step, is_epoch= True)

...
def clip_loss_pointwise(
    logits,
    num_negatives,
):
    N = logits.shape[0] 
    assert N % (num_negatives + 1) == 0, "Number of samples N must be a multiple of (num_negatives + 1)"
    logits_reshaped = logits.view(N // (num_negatives + 1), (num_negatives + 1))
    target = torch.zeros(N // (num_negatives + 1), dtype=torch.long, device=logits_reshaped.device)
...
def grad_cache_loss(tower1, t1_inputs, tower2, t2_inputs, chunk_size, logit_scale, bidirectional=False):
    ...
    for chunk_start in range(0, t1_inputs["input_ids"].shape[0], chunk_size):
        query_chunk = {k: v[chunk_start : chunk_start + chunk_size] for k, v in t1_inputs.items()}
        chunked_queries.append(query_chunk)
    
    for chunk_start in range(0, t2_inputs["input_ids"].shape[0], chunk_size):

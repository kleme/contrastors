...
class BiEncoder(PreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_fused_kernels:
            ...
            if getattr(config, "nomic_encoder", True) is False:
                ...
                if os.path.exists(config.model_name):
                    from contrastors.models.biencoder.configuration_biencoder import BiEncoderConfig
                    model_config = BiEncoderConfig.from_pretrained(config.model_name)
                    self.trunk = self.from_pretrained(config.model_name, config = model_config).trunk
                else:
                    model_config = AutoConfig.from_pretrained(config.model_name, trust_remote_code=True)
                    ...
                    if config.pretrained:
                        ...
                        self.trunk = NomicBertModel(config=model_config, trust_remote_code=True, add_pooling_layer=False)
        else:
            if "snowflake" in config.model_name:
                self.trunk = AutoModel.from_pretrained(config.model_name, trust_remote_code=True, add_pooling_layer=False, safe_serialization=True)
            else:
                self.trunk = AutoModel.from_pretrained(config.model_name, trust_remote_code=True, attn_implementation="flash_attention_2")
            # if "codesage" in "codesage/codesage-small":
            if "codesage" in config.model_name:
                print("Adding EOS token into Tokenizer")
                self.tokenizer = AutoTokenizer.from_pretrained(config.model_name, trust_remote_code=True, add_eos_token=True)
            else:
        ...
        if config.gradient_checkpointing:
            self.trunk.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
        ...
        elif config.pooling == "pooler_output":
            self.selector = "pooler_output"
    ...
    def forward(self, input_ids, attention_mask=None, is_padded_inputs=True, normalize=True, binarize=False, **kwargs):
        ...
        with context():
            if self.selector == "pooler_output":
                attention_mask = input_ids.ne(self.tokenizer.pad_token_id).to(input_ids.device)
                trunk_output = self.trunk(input_ids, attention_mask=attention_mask, return_dict=True, **kwargs)
            else:
...
class SimpleJointBiEncoder(PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        if config.use_fused_kernels:
            ...
            if getattr(config, "nomic_encoder", True) is False:
                ...
                if model_config.model_type != "nomic_bert":
                    ...
                    model_config = bert_config_to_nomic_config(model_config)
                if config.pretrained:
                    self.trunk_text = NomicBertModel.from_pretrained(
                        config.model_name,
                        add_pooling_layer=False,
                        config=model_config,
                        # don't train with dynamic NTK rotary
                    ...
                    self.trunk_code = NomicBertModel.from_pretrained(
                        config.model_name,
                        add_pooling_layer=False,
                        config=model_config,
                        # don't train with dynamic NTK rotary
                    ...
                    self.trunk_text = NomicBertModel(config=model_config, trust_remote_code=True, add_pooling_layer=False)
                    self.trunk_code = NomicBertModel(config=model_config, trust_remote_code=True, add_pooling_layer=False)
        else:
            raise NotImplementedError
            if "snowflake" in config.model_name:
                self.trunk = AutoModel.from_pretrained(config.model_name, trust_remote_code=True, add_pooling_layer=False, safe_serialization=True)
    ...
    def forward(self, input_ids, attention_mask=None, is_padded_inputs=True, normalize=True, binarize=False, is_code = False, **kwargs):
        if is_code:
            return self.code_forward(input_ids, attention_mask, is_padded_inputs, normalize, binarize, **kwargs)
        
        return self.text_forward(input_ids, attention_mask, is_padded_inputs, normalize, binarize, **kwargs)

    def text_forward(self, input_ids, attention_mask=None, is_padded_inputs=True, normalize=True, binarize=False, **kwargs):
        context = torch.no_grad if self.frozen_trunk else nullcontext
        with context():
            if self.selector == "pooler_output":
                attention_mask = input_ids.ne(self.tokenizer.pad_token_id).to(input_ids.device)
                trunk_output = self.trunk_text(input_ids, attention_mask=attention_mask, return_dict=True, **kwargs)
            else:
                 trunk_output = self.trunk_text(input_ids, attention_mask=attention_mask, **kwargs)
    ...
    def code_forward(self, input_ids, attention_mask=None, is_padded_inputs=True, normalize=True, binarize=False, **kwargs):
        context = torch.no_grad if self.frozen_trunk else nullcontext
        with context():
            if self.selector == "pooler_output":
                attention_mask = input_ids.ne(self.tokenizer.pad_token_id).to(input_ids.device)
                trunk_output = self.trunk_code(input_ids, attention_mask=attention_mask, return_dict=True, **kwargs)
            else:
                 trunk_output = self.trunk_code(input_ids, attention_mask=attention_mask, **kwargs)

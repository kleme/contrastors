...
def filter_dataset(dataset, output_dir, ids_to_keep):
    path = Path(dataset)
    if path.is_dir():
...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained(model_name, trust_remote_code=True).to('cuda').to(torch.float16)
    ...
    for i in tqdm(range(num_iterations), disable=dist.get_rank() != 0):
        ...
        dist.barrier()
    filter_dataset(args.dataset, args.output_dir, ids2keep)

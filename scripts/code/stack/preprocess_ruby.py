...
# class RubyProcessor(BasePreprocessor):
...
#                     else:
#                         if node.children[i].type in ('method'):
...
#         RubyProcessor.get_functions_iterative(root_node, functions, docstrings)
...
# for i in range(3):
#     ds = load_dataset(f"nomic-ai/the-stack-v2-dedup-Ruby-{i}", split = 'train')
...
#     ds.push_to_hub(f"nomic-uiuc/the-stack-v2-dedup-Ruby-{i}-processed", private=True)
    
# ds = load_dataset("nomic-ai/the-stack-v2-dedup-Ruby-0", split = 'train')
...
# output_dir = Path(f"{CONTRASTIVE_CODE_DIR}/stack_v0_ruby_mini/")

...
# class PHPProcessor(BasePreprocessor):
...
#                         docstr = PHPProcessor.process_docstring(strip_c_style_comment_delimiters(inner_docstring))
#                         if docstr:
#                             docstrings.append(docstr)
...
#                     else:
#                         if node.children[i].type in ('function_definition', 'method_declaration'):
...
#         PHPProcessor.get_functions_iterative(root_node, functions, docstrings)
...
# for i in range(8):
#     ds = load_dataset(f"nomic-ai/the-stack-v2-dedup-PHP-{i}", split = 'train')
...
#     ds.push_to_hub(f"nomic-uiuc/the-stack-v2-dedup-PHP-{i}-processed", private=True)
    
# ds = load_dataset("nomic-ai/the-stack-v2-dedup-PHP-0", split = 'train')
...
# output_dir = Path(f"{CONTRASTIVE_CODE_DIR}/stack_v0_php_v0")
